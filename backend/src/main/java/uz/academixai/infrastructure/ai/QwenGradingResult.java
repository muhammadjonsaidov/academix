...
/**
 * academix_tz.md §3.2 — combined grading + plagiarism, one Qwen call (cost-optimization: was 2
 * calls, now 1). Deliberately has no {@code aiScorePercent} — the AI never computes a final
 * percentage; {@code AIAnalysisService} derives it from {@code criteriaScores} via weighted-sum for
 * auditability/consistency.
 */
public record QwenGradingResult(
